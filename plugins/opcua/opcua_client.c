...
static void opcua_client_state_callback(UA_Client *client, 
    ...
    if (sessionState == UA_SESSIONSTATE_ACTIVATED) {
        if (!opcua_client->connected) {
            ...
            if (plugin) {
                plog_notice(plugin, "【连接成功】OPC UA连接已成功建立，会话已激活，通道状态: %d, 会话状态: %d, 状态码: 0x%08x，耗时: %ld ms", 
        ...
        if (opcua_client->connected) {
            ...
            if (plugin) {
                plog_warn(plugin, "【连接断开】OPC UA连接已断开，通道状态: %d, 会话状态: %d, 连接状态码: 0x%08x", 
            ...
            if (connectStatus != UA_STATUSCODE_GOOD) {
                ...
                if (plugin) {
                    plog_warn(plugin, "【连接失败】OPC UA连接尝试失败，通道状态: %d, 会话状态: %d, 连接状态码: 0x%08x，耗时: %ld ms", 
                             channelState, sessionState, connectStatus, duration);
                }
            }
        }
...
static void 
opcua_log_callback(void *context, UA_LogLevel level, UA_LogCategory category, 
                         ...
                         opcua_security_mode_e security_mode)
{
    ...
    // 添加连接开始日志
    if (client->plugin) {
        plog_info(client->plugin, "Connecting to OPC UA server: %s", endpoint_url);
    }

    UA_ClientConfig *config = UA_Client_getConfig(client->client);
    if (client->plugin != NULL) {
        // plog_info(client->plugin, "OPC UA client created with default configuration");
        // config->logging->log = opcua_log_callback;
        // config->logging->context = client->plugin;
    }
    //UA_ClientConfig_setDefault(config);
    ...
    if (username != NULL && password != NULL && strlen(username) > 0) {
        if (client->plugin) {
            plog_debug(client->plugin, "Using username authentication for OPC UA connection");
        }
    ...
    case OPCUA_SECURITY_MODE_NONE:
        // 默认无安全策略，不需要额外配置
        if (client->plugin) {
            plog_debug(client->plugin, "Using None security mode for OPC UA connection");
        }
        break;
    case OPCUA_SECURITY_MODE_SIGN:
        ...
        if (certificate != NULL && private_key != NULL) {
            if (client->plugin) {
                plog_debug(client->plugin, "Using Sign security mode for OPC UA connection");
            }
            // 在实际应用中，这里需要加载证书和私钥
            // 设置消息安全模式为签名
            ...
            // 缺少证书或私钥，无法进行签名
            if (client->plugin) {
                plog_error(client->plugin, "Missing certificate or private key for Sign security mode");
            }
            return -1;
        }
        break;
    case OPCUA_SECURITY_MODE_SIGN_ENCRYPT:
        ...
        if (certificate != NULL && private_key != NULL) {
            if (client->plugin) {
                plog_debug(client->plugin, "Using SignAndEncrypt security mode for OPC UA connection");
            }
            // 设置消息安全模式为签名和加密
            ...
            // 缺少证书或私钥，无法进行签名和加密
            if (client->plugin) {
                plog_error(client->plugin, "Missing certificate or private key for SignAndEncrypt security mode");
            }
            return -1;
        }
        break;
    default:
        // 未知安全模式
        if (client->plugin) {
            plog_error(client->plugin, "Unknown security mode: %d", security_mode);
        }
        return -1;
    }
    ...
    if (status != UA_STATUSCODE_GOOD) {
        if (client->plugin) {
            plog_error(client->plugin, "Failed to connect to OPC UA server: %s, status: 0x%08x", 
                      endpoint_url, status);
        }
        return -1;
    }
    ...
    if (client->plugin) {
        plog_notice(client->plugin, "Successfully connected to OPC UA server: %s", endpoint_url);
    }
...
void opcua_client_disconnect(opcua_client_t *client)
{
    ...
    if (client->plugin) {
        plog_debug(client->plugin, "Successfully disconnected from OPC UA server");
    }
                           ...
                           size_t *array_indices, size_t num_indices)
{
    ...
    UA_StatusCode status = UA_Client_readValueAttribute(client->client, ua_node_id, &variant);
    ...
    // 布尔值转换
    if (variant.type == &UA_TYPES[UA_TYPES_BOOLEAN] && data_type == &UA_TYPES[UA_TYPES_BYTE]) {
        // 布尔值转字节 (Boolean -> UINT8)
        *(UA_Byte*)value = *(UA_Boolean*)variant.data ? 1 : 0;
    } else if (variant.type == &UA_TYPES[UA_TYPES_BYTE] && data_type == &UA_TYPES[UA_TYPES_BOOLEAN]) {
        // 字节转布尔值 (UINT8 -> Boolean)
        *(UA_Boolean*)value = *(UA_Byte*)variant.data != 0;
    } 
    // 整数类型转换
    else if (variant.type == &UA_TYPES[UA_TYPES_SBYTE] && data_type == &UA_TYPES[UA_TYPES_BYTE]) {
        // SByte转Byte (INT8 -> UINT8)
        *(UA_Byte*)value = (UA_Byte)*(UA_SByte*)variant.data;
    } else if (variant.type == &UA_TYPES[UA_TYPES_BYTE] && data_type == &UA_TYPES[UA_TYPES_SBYTE]) {
        // Byte转SByte (UINT8 -> INT8)
        *(UA_SByte*)value = (UA_SByte)*(UA_Byte*)variant.data;
    } else if (variant.type == &UA_TYPES[UA_TYPES_INT16] && data_type == &UA_TYPES[UA_TYPES_UINT16]) {
        // INT16转UINT16
        *(UA_UInt16*)value = (UA_UInt16)*(UA_Int16*)variant.data;
    } else if (variant.type == &UA_TYPES[UA_TYPES_UINT16] && data_type == &UA_TYPES[UA_TYPES_INT16]) {
        // UINT16转INT16
        *(UA_Int16*)value = (UA_Int16)*(UA_UInt16*)variant.data;
    } else if (variant.type == &UA_TYPES[UA_TYPES_INT32] && data_type == &UA_TYPES[UA_TYPES_UINT32]) {
        // INT32转UINT32
        *(UA_UInt32*)value = (UA_UInt32)*(UA_Int32*)variant.data;
    } else if (variant.type == &UA_TYPES[UA_TYPES_UINT32] && data_type == &UA_TYPES[UA_TYPES_INT32]) {
        // UINT32转INT32
        *(UA_Int32*)value = (UA_Int32)*(UA_UInt32*)variant.data;
    } else if (variant.type == &UA_TYPES[UA_TYPES_INT64] && data_type == &UA_TYPES[UA_TYPES_UINT64]) {
        // INT64转UINT64
        *(UA_UInt64*)value = (UA_UInt64)*(UA_Int64*)variant.data;
    } else if (variant.type == &UA_TYPES[UA_TYPES_UINT64] && data_type == &UA_TYPES[UA_TYPES_INT64]) {
        // UINT64转INT64
        *(UA_Int64*)value = (UA_Int64)*(UA_UInt64*)variant.data;
    } 
    // 浮点数类型转换
    else if (variant.type == &UA_TYPES[UA_TYPES_FLOAT] && data_type == &UA_TYPES[UA_TYPES_DOUBLE]) {
        // FLOAT转DOUBLE
        *(UA_Double*)value = (UA_Double)*(UA_Float*)variant.data;
    } else if (variant.type == &UA_TYPES[UA_TYPES_DOUBLE] && data_type == &UA_TYPES[UA_TYPES_FLOAT]) {
        // DOUBLE转FLOAT
        *(UA_Float*)value = (UA_Float)*(UA_Double*)variant.data;
    } 
    // 特殊类型转换
    else if (variant.type == &UA_TYPES[UA_TYPES_DATETIME] && data_type == &UA_TYPES[UA_TYPES_UINT32]) {
        // DateTime转UINT32 (时间戳转换为秒)
        UA_DateTime dt = *(UA_DateTime*)variant.data;
        // 将OPC UA时间戳（1601年1月1日以来的100纳秒数）转换为Unix时间戳（1970年1月1日以来的秒数）
        // OPC UA时间戳比Unix时间戳早11644473600秒
        *(UA_UInt32*)value = (UA_UInt32)((dt / 10000000ULL) - 11644473600ULL);
    } else if (variant.type == &UA_TYPES[UA_TYPES_LOCALIZEDTEXT] && data_type == &UA_TYPES[UA_TYPES_STRING]) {
                           ...
                           size_t *array_indices, size_t num_indices)
{
    ...
    // 写入值
    UA_StatusCode status = UA_Client_writeValueAttribute(client->client, uaNodeId, &variant);
    ...
    if (status != UA_STATUSCODE_GOOD) {
        if (client->plugin) {
            plog_error(client->plugin, "写入OPC UA节点失败: %s, 状态码: %d", node_id, status);
        }
        return -1;
    }
...
char *opcua_client_node_id_to_string(const UA_NodeId *node_id)
{
    ...
    case UA_NODEIDTYPE_NUMERIC:
        ...
        if (result) {
            if (node_id->namespaceIndex != 0) {
                // 非默认命名空间，包含ns=前缀
                snprintf(result, 30, "ns=%hu;i=%u", 
                         node_id->namespaceIndex, node_id->identifier.numeric);
            } else {
                // 默认命名空间，可以省略ns=0
    ...
    case UA_NODEIDTYPE_GUID:
        // GUID类型，格式：ns=命名空间;g=GUID
        {
            ...
            if (result) {
                if (node_id->namespaceIndex != 0) {
                    // 非默认命名空间，包含ns=前缀
                    snprintf(result, 60, "ns=%hu;g=%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
                         node_id->namespaceIndex,
    ...
    case UA_NODEIDTYPE_BYTESTRING:
        // 二进制数据类型，格式：ns=命名空间;b=十六进制字符串
        {
            ...
            if (result) {
                ...
                if (node_id->namespaceIndex != 0) {
                    // 非默认命名空间，包含ns=前缀
                    pos += snprintf(pos, 30, "ns=%hu;b=", node_id->namespaceIndex);
                } else {
                    // 默认命名空间，可以省略ns=0
                ...
                for (size_t i = 0; i < len; i++) {
                    pos += snprintf(pos, 3, "%02x", node_id->identifier.byteString.data[i]);
                }
            }
        }
        break;
    ...
    default:
        ...
        result = malloc(50);
        if (result) {
            snprintf(result, 50, "未知类型的节点ID (类型: %d)", node_id->identifierType);
        }
        break;
    }
...
// 浏览节点实现
int opcua_client_browse_node(opcua_client_t *client, const UA_NodeId *node_id, UT_array *results)
{
    ...
    if (bReq.nodesToBrowse == NULL) {
        if (client->plugin) {
            plog_error(client->plugin, "Failed to allocate memory for browse description");
            if (nodeIdStr) free(nodeIdStr);
        }
        return -1;
    }
    ...
    if (bResp.resultsSize != 1) {
        if (client->plugin) {
            plog_error(client->plugin, "Browse response has unexpected result size: %zu", 
                     bResp.resultsSize);
            if (nodeIdStr) free(nodeIdStr);
        }
    ...
    if (bResp.results[0].statusCode != UA_STATUSCODE_GOOD) {
        if (client->plugin) {
            plog_error(client->plugin, "Browse result has error, status: 0x%08x", 
    ...
    for (size_t i = 0; i < referencesCount; i++) {
        ...
        if (client->plugin) {
            ...
            if (result.displayName.data && result.displayName.length > 0) {
                ...
                if (displayName) {
                    memcpy(displayName, result.displayName.data, result.displayName.length);
            ...
            switch (result.nodeClass) {
                case UA_NODECLASS_OBJECT: strcpy(nodeClassStr, "Object"); break;
                case UA_NODECLASS_VARIABLE: strcpy(nodeClassStr, "Variable"); break;
                case UA_NODECLASS_METHOD: strcpy(nodeClassStr, "Method"); break;
                case UA_NODECLASS_OBJECTTYPE: strcpy(nodeClassStr, "ObjectType"); break;
                case UA_NODECLASS_VARIABLETYPE: strcpy(nodeClassStr, "VariableType"); break;
                case UA_NODECLASS_REFERENCETYPE: strcpy(nodeClassStr, "ReferenceType"); break;
                case UA_NODECLASS_DATATYPE: strcpy(nodeClassStr, "DataType"); break;
                case UA_NODECLASS_VIEW: strcpy(nodeClassStr, "View"); break;
                default: strcpy(nodeClassStr, "Unknown");
            }
            
            plog_debug(client->plugin, "Browse result[%zu]: Node=%s, Name=%s, Class=%s", 
                     i, childNodeStr ? childNodeStr : "unknown", 
                     displayName ? displayName : "unknown", nodeClassStr);
            
            if (childNodeStr) free(childNodeStr);
...
static neu_type_e opcua_datatype_nodeid_to_neuron_type(const UA_NodeId *dataTypeId) {
    // 安全检查
    if (dataTypeId == NULL) {
...
{
    if (client == NULL || nodeId == NULL || type == NULL || !client->connected) {
        if (client && client->plugin) {
            plog_error(client->plugin, "Invalid parameters for read_variable_datatype");
        }
        return -1;
    }
    ...
    if (client->plugin) {
        ...
        if (nodeIdStr) {
            plog_debug(client->plugin, "Reading DataType attribute for node: %s", nodeIdStr);
        }
    }
    ...
    *type = NEU_TYPE_STRING;
    ...
    // 首先尝试读取DataType属性
    UA_StatusCode status = UA_Client_readDataTypeAttribute(client->client, *nodeId, &dataTypeId);
    ...
    if (status == UA_STATUSCODE_GOOD) {
        ...
        *type = opcua_datatype_nodeid_to_neuron_type(&dataTypeId);
        ...
        if (client->plugin) {
            ...
            if (typeIdStr) {
                plog_debug(client->plugin, "Node %s dataType NodeId: %s, Neuron type: %d", 
        ...
        if (client->plugin) {
            plog_debug(client->plugin, "Failed to read DataType attribute, status: %d", status);
        }
    }
    
    // 然后检查ValueRank属性，判断是否为数组
    UA_Int32 valueRank = -1;
    status = UA_Client_readValueRankAttribute(client->client, *nodeId, &valueRank);
    ...
    if (status == UA_STATUSCODE_GOOD) {
        ...
        if (valueRank > 0) {
            isArray = UA_TRUE;
            if (client->plugin) {
                plog_debug(client->plugin, "Node %s is an array with valueRank = %d", 
                          nodeIdStr ? nodeIdStr : "unknown", valueRank);
            }
        }
    } else if (client->plugin) {
        plog_debug(client->plugin, "Failed to read ValueRank attribute, status: %d", status);
    }
    ...
    if (!isArray || status != UA_STATUSCODE_GOOD) {
        ...
        if (status == UA_STATUSCODE_GOOD) {
            // 检查是否为数组
            if (value.arrayLength > 0 || (value.arrayDimensionsSize > 0 && value.arrayDimensions && value.arrayDimensions[0] > 0)) {
                isArray = UA_TRUE;
                if (client->plugin) {
                    plog_debug(client->plugin, "Detected array by reading value, array length: %d", 
                              (int)value.arrayLength);
                }
            }
            
            // 如果未能通过DataType属性获取类型，尝试从值获取
            if (*type == NEU_TYPE_STRING && value.type && !UA_NodeId_isNull(&value.type->typeId)) {
                *type = opcua_datatype_nodeid_to_neuron_type(&value.type->typeId);
                if (client->plugin) {
                    char *valueTypeIdStr = opcua_client_node_id_to_string(&value.type->typeId);
                    if (valueTypeIdStr) {
                        plog_debug(client->plugin, "Detected type from value: %s, Neuron type: %d", 
                                  valueTypeIdStr, *type);
                        free(valueTypeIdStr);
                    }
                }
    ...
    // 如果是数组，将类型设置为相应的数组类型
        ...
        if (client->plugin) {
            plog_debug(client->plugin, "Node %s is array type with base type %d", 
        ...
        switch (base_type) {
            ...
            default:
                // 对于其他类型，保持原类型不变
                if (client->plugin) {
                    plog_warn(client->plugin, "Node %s is array of unsupported type %d, keeping original type", 
        ...
        if (client->plugin) {
            plog_debug(client->plugin, "Node %s array type mapped to Neuron type %d", 
                      nodeIdStr ? nodeIdStr : "unknown", *type);
        }
    }
...
{
    if (client == NULL || nodeId == NULL || value == NULL || !client->connected) {
                        ...
                        opcua_security_mode_e security_mode)
{
    ...
    // 添加连接开始日志
    if (client->plugin) {
        plog_info(client->plugin, "正在异步连接到OPC UA服务器: %s", endpoint_url);
    }
    ...
    case OPCUA_SECURITY_MODE_SIGN_ENCRYPT:
        if (certificate != NULL && private_key != NULL) {
            if (client->plugin) {
                plog_debug(client->plugin, "使用SignAndEncrypt安全模式进行OPC UA连接");
            }
            config->securityMode = UA_MESSAGESECURITYMODE_SIGNANDENCRYPT;
        } else {
            if (client->plugin) {
                plog_error(client->plugin, "缺少证书或私钥进行SignAndEncrypt安全模式");
            }
            return -1;
        }
        break;
    ...
    if (status != UA_STATUSCODE_GOOD) {
        client->connecting = false;
        if (client->plugin) {
            plog_error(client->plugin, "启动异步连接到OPC UA服务器失败: %s, 状态码: 0x%08x", 
                      endpoint_url, status);
        }
        return -1;
    }
    ...
    if (client->plugin) {
        plog_notice(client->plugin, "已成功启动到OPC UA服务器的异步连接: %s", endpoint_url);
    }
...
int opcua_client_process_async(opcua_client_t *client, uint32_t timeout_ms)
{
    ...
    if (status != UA_STATUSCODE_GOOD) {
        if (client->plugin) {
            plog_error(client->plugin, "处理OPC UA客户端异步操作失败, 状态码: 0x%08x", status);
        }
        return -1;
    }
    ...
    UA_SessionState sessionState;
    UA_StatusCode connectStatus;
    UA_Client_getState(client->client, &channelState, &sessionState, &connectStatus);
    ...
    if (new_state != client->connected) {
        if (new_state) {
            ...
            if (client->plugin) {
                plog_notice(client->plugin, "OPC UA连接已成功建立，会话已激活，通道状态: %d, 会话状态: %d, 状态码: 0x%08x，耗时: %ld ms", 
                          channelState, sessionState, connectStatus, duration);
            }
        } else {
            // 连接状态从连接变为断开
            client->connected = false;
            if (client->plugin) {
                plog_warn(client->plugin, "OPC UA连接已主动检测到断开，通道状态: %d, 会话状态: %d, 连接状态码: 0x%08x", 
                         channelState, sessionState, connectStatus);
            }
        }
    } else if (client->connecting && sessionState == UA_SESSIONSTATE_ACTIVATED) {
        // 连接中状态已变为已连接
        client->connecting = false;
        client->connected = true;
        // 计算连接时间
        ...
        if (client->plugin) {
            plog_notice(client->plugin, "OPC UA连接过程完成，会话已激活，通道状态: %d, 会话状态: %d, 状态码: 0x%08x，耗时: %ld ms", 
                      channelState, sessionState, connectStatus, duration);
        }
    }
    ...
    if (client->connecting) {
        ...
        if (client->connect_start_time > 0) {
            ...
            if (duration > CONNECTION_TIMEOUT_MS) {
                ...
                if (client->plugin) {
                    plog_error(client->plugin, "异步连接到OPC UA服务器超时，已经尝试了%ld ms (%ld 秒)", 
                              duration, duration/1000);
                    
                    // 获取服务器状态信息帮助诊断
                    plog_error(client->plugin, "连接失败状态: 通道=%d, 会话=%d, 状态码=0x%08x", 
                              channelState, sessionState, connectStatus);
                }
                return -1;
            }
            ...
            if (duration > 3000 && duration % 3000 < 100) { // 每3秒打印一次
                if (client->plugin) {
                    plog_notice(client->plugin, "正在连接OPC UA服务器，已经尝试了 %ld 秒... (通道=%d, 会话=%d, 状态码=0x%08x)", 
                              duration/1000, channelState, sessionState, connectStatus);
                }
            }
        }
