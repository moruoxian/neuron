...
static int driver_test_read_tag(neu_plugin_t *plugin, void *req,
                                neu_datatag_t tag);
static int driver_add_tags(neu_plugin_t *plugin, const char *group,
...
const neu_plugin_module_t neu_plugin_module = {
    ...
    .module_descr_zh = "该插件用于连接 OPC UA 服务器。"
                       "支持用户名/密码和证书认证。",
...
static int driver_init(neu_plugin_t *plugin, bool load)
{
    ...
    // 注册延时度量指标
    NEU_PLUGIN_REGISTER_METRIC(plugin, NEU_METRIC_LAST_RTT_MS,
...
static int driver_start(neu_plugin_t *plugin)
{
    ...
    plog_notice(plugin, "Starting OPC UA client and connecting to server: %s",
                plugin->endpoint_url);

    // 使用异步方式连接到服务器
    int ret = opcua_client_connect_async(
        plugin->client, plugin->endpoint_url, plugin->username,
        plugin->password, plugin->certificate, plugin->private_key,
        plugin->security_mode);

    if (ret != 0) {
        plog_error(plugin,
                   "Failed to start async connection to OPC UA server: %s",
                   plugin->endpoint_url);

        // 如果启用了自动重连，虽然连接失败，但仍然启动插件
        if (plugin->reconnect_enabled) {
            plog_notice(plugin,
                        "Auto reconnect is enabled, plugin will start and try "
                        "to reconnect later");
    ...
    while (connect_wait_count > 0) {
        ...
        if (connect_status > 0) {
            ...
            plugin->reconnect_attempts = 0; // 重置重连计数
            plog_notice(plugin, "Successfully connected to OPC UA server: %s",
                        plugin->endpoint_url);
            return 0;
        }
        // 连接失败
    ...
    if (plugin->reconnect_enabled) {
        plog_notice(plugin,
                    "OPC UA connection not established yet, but auto reconnect "
                    "is enabled. Plugin will start and connection will "
    ...
    plog_error(
        plugin,
        "Failed to connect to OPC UA server: %s and auto reconnect is disabled",
        plugin->endpoint_url);
    plugin->connected         = false;
    plugin->common.link_state = NEU_NODE_LINK_STATE_DISCONNECTED;
    return -1;
...
static int driver_config(neu_plugin_t *plugin, const char *config)
{
    int             ret           = 0;
    char *          err_param     = NULL;
    neu_json_elem_t endpoint_url  = { .name = "endpoint_url",
                                     .t    = NEU_JSON_STR };
    neu_json_elem_t username      = { .name = "username", .t = NEU_JSON_STR };
    neu_json_elem_t password      = { .name = "password", .t = NEU_JSON_STR };
    neu_json_elem_t security_mode = { .name = "security_mode",
                                      .t    = NEU_JSON_INT };
    neu_json_elem_t certificate = { .name = "certificate", .t = NEU_JSON_STR };
    neu_json_elem_t private_key = { .name = "private_key", .t = NEU_JSON_STR };
    neu_json_elem_t reconnect_enabled      = { .name = "reconnect_enabled",
                                          .t    = NEU_JSON_INT };
    neu_json_elem_t reconnect_interval     = { .name = "reconnect_interval",
                                           .t    = NEU_JSON_INT };
    neu_json_elem_t max_reconnect_attempts = { .name = "max_reconnect_attempts",
                                               .t    = NEU_JSON_INT };

    ret = neu_parse_param((char *) config, &err_param, 9, &endpoint_url,
                          &username, &password, &security_mode, &certificate,
                          &private_key, &reconnect_enabled, &reconnect_interval,
    ...
    default:
        plugin->security_mode = OPCUA_SECURITY_MODE_NONE;
        plog_notice(plugin, "Unknown security mode: %ld, using None",
                    security_mode.v.val_int);
        break;
    }
    ...
    plugin->max_reconnect_attempts =
        (uint32_t) max_reconnect_attempts.v.val_int;
    ...
    plog_notice(plugin, "Auto reconnect: %s, interval: %u ms, max attempts: %u",
                ...
                plugin->reconnect_interval, plugin->max_reconnect_attempts);
...
static int try_reconnect(neu_plugin_t *plugin)
{
    ...
    if (plugin->max_reconnect_attempts > 0) {
        snprintf(max_attempts_str, sizeof(max_attempts_str), "%u",
                 plugin->max_reconnect_attempts);
    } else {
    ...
    plog_notice(plugin, "已成功启动到OPC UA服务器的异步连接: %s",
                plugin->endpoint_url);
...
static int driver_group_timer(neu_plugin_t *plugin, neu_plugin_group_t *group)
{
    ...
    if (plugin->client) {
        // 处理异步操作，获取处理结果
        int async_result =
            opcua_client_process_async(plugin->client, 10); // 10ms超时
    ...
    if (!plugin->connected && plugin->reconnect_enabled) {
        ...
        } else if (plugin->reconnect_attempts >=
                   plugin->max_reconnect_attempts) {
    ...
    if (gd == NULL) {
        ...
        utarray_foreach(group->tags, neu_datatag_t *, tag)
        {
            ...
            if (ret != 0) {
                free(point);
                plog_error(plugin, "Failed to convert tag to point: %s",
                           tag->name);
                continue;
            }

            utarray_push_back(gd->tags, &point);
            plog_debug(plugin, "Added tag %s to group %s", point->name,
                       group->group_name);
        }
    ...
    if (gd->cmd_sort == NULL) {
        ...
        if (gd->cmd_sort == NULL) {
            plog_error(plugin, "Failed to create command sort for group: %s",
                       group->group_name);
            return -1;
        }
    }
    ...
    for (uint16_t i = 0; i < gd->cmd_sort->cmd_size; i++) {
        utarray_foreach(gd->tags, opcua_point_t **, p_tag)
        {
            ...
            if (value == NULL) {
                plog_error(plugin,
                           "Failed to allocate memory for tag value: %s",
                           tag->name);
                continue;
            }
            ...
            if (ret != 0) {
                ...
                plog_error(plugin,
                           "Failed to read value for tag: %s, node_id: %s",
    ...
    if (success_count > 0) {
        NEU_PLUGIN_UPDATE_METRIC(plugin, NEU_METRIC_LAST_RTT_MS, duration_ms,
                                 NULL);
        plog_debug(plugin,
                   "Group %s read complete. Duration: %ld ms, Success: %d/%d",
        ...
        NEU_PLUGIN_UPDATE_METRIC(plugin, NEU_METRIC_LAST_RTT_MS,
                                 NEU_METRIC_LAST_RTT_MS_MAX, NULL);
                        ...
                        neu_value_u value)
{
    ...
    // 将Neuron值转换为OPC UA值
    ret =
        opcua_convert_neu_to_value(&value, tag->type, ua_value, point.ua_type);
    if (ret != 0) {
    ...
    // 写入值
    plog_debug(plugin, "正在写入值到OPC UA节点: %s (%s)", point.name,
               point.node_id);
    ret = opcua_client_write_value(plugin->client, point.node_id, point.ua_type,
                                   ua_value, NULL, 0);
...
static int driver_write_tags(neu_plugin_t *plugin, void *req, UT_array *tags)
{
    ...
    utarray_foreach(tags, neu_plugin_tag_value_t *, tag_value)
    {
        ...
        // 将Neuron值转换为OPC UA值
        ret = opcua_convert_neu_to_value(
            &tag_value->value, tag_value->tag->type, ua_value, point.ua_type);
        if (ret != 0) {
        ...
        plog_debug(plugin, "正在写入值到OPC UA节点: %s (%s)", point.name,
                   point.node_id);
        ret = opcua_client_write_value(plugin->client, point.node_id,
                                       point.ua_type, ua_value, NULL, 0);
    ...
    if (success_count == total_count) {
        ...
        plog_warn(plugin, "部分写入成功: %d/%d 个标签", success_count,
                  total_count);
        plugin->common.adapter_callbacks->driver.write_response(
                                ...
                                neu_datatag_t tag)
{
    ...
    // Read value
    ret = opcua_client_read_value(plugin->client, point.node_id, point.ua_type,
                                  value, NULL, 0);
...
#pragma GCC diagnostic ignored "-Wformat-truncation"
    ...
    // 处理ID不为空的情况
        ...
        if (is_array_element_access) {
            ...
            node_id = opcua_client_parse_node_id(
                plugin->client, array_base_node_id, NULL, NULL);
        } else {
            node_id =
                opcua_client_parse_node_id(plugin->client, id, NULL, NULL);
        }
        ...
        if (opcua_client_read_value_attribute(plugin->client, &node_id,
            ...
            if (!UA_Variant_isScalar(&value) && value.arrayLength > 0) {
                ...
                plog_notice(plugin, "检测到数组节点: %s, 长度=%lu, 类型=%d", id,
                            (unsigned long) array_length, array_element_type);
            }
        ...
        if (is_array_node && array_length > 0) {
            plog_notice(plugin, "生成数组元素列表，数组长度: %lu",
            ...
            if (status == UA_STATUSCODE_GOOD && displayName.text.length > 0) {
                ...
                plog_debug(plugin, "使用节点显示名称作为数组前缀: %s",
                           array_node_name);
            } else {
                ...
                if (id != NULL && strlen(id) > 0) {
                    ...
                    // 选择最后出现的分隔符作为名称起点
                    if (last_slash &&
                        (!effective_pos || last_slash > effective_pos))
                        effective_pos = last_slash + 1;
                    if (last_colon &&
                        (!effective_pos || last_colon > effective_pos))
                        effective_pos = last_colon + 1;
                    if (last_semi &&
                        (!effective_pos || last_semi > effective_pos))
                        effective_pos = last_semi + 1;

                    if (effective_pos) {
                        // 获取名称部分，去掉后缀数字和特殊字符
                        size_t name_len = strlen(effective_pos);
                        size_t copy_len = name_len < NEU_TAG_NAME_LEN - 1
                            ? name_len
                            : NEU_TAG_NAME_LEN - 1;
                ...
                plog_debug(plugin, "无法获取节点显示名称，使用替代名称: %s",
                           array_node_name);
            }
            ...
            if (semi_pos) {
                ...
                for (size_t i = 0; i < array_length; i++) {
                    ...
                    snprintf(array_element.id, sizeof(array_element.id),
                             "%s[%lu]%s", ns_part, (unsigned long) i, id_part);
                    ...
                    plog_debug(plugin, "添加数组元素: 名称=%s, ID=%s, 类型=%d",
                               array_element.name, array_element.id,
                               array_element.type);
                }
            } else {
                ...
                for (size_t i = 0; i < array_length; i++) {
                    ...
                    // 使用原始格式: "nodeID[索引]"
                    snprintf(array_element.id, sizeof(array_element.id),
                             "%s[%lu]", base_node_id, (unsigned long) i);
        ...
        // 处理数组元素访问的情况
            ...
            if (opcua_client_read_value_attribute(plugin->client, &node_id,
                ...
                UA_STATUSCODE_GOOD) {
                // 确认是数组并且索引在范围内
                if (!UA_Variant_isScalar(&element_value) &&
                    element_value.arrayLength > (size_t) array_index) {
                    // 获取单个元素并设置为结果
                    ...
                    plog_notice(plugin,
                                "已访问数组元素: 基础节点=%s, 索引=%d, 类型=%d",
                                array_base_node_id, array_index, resp.type);
                } else {
                    plog_error(
                        plugin,
                        "数组索引超出范围或不是数组: 索引=%d, 数组长度=%lu",
            ...
            if (browse_results == NULL) {
                plog_error(plugin,
                           "Failed to allocate memory for browse results");
                resp.error = NEU_ERR_EINTERNAL;
                UA_NodeId_clear(&node_id);
                goto response;
            }
            ...
            // 浏览节点
            if (opcua_client_browse_node(plugin->client, &node_id,
                                         browse_results) != 0) {
            ...
            utarray_foreach(browse_results, opcua_browse_result_t *, result)
            {
                ...
                scan_tag.tag =
                    (result->nodeClass == UA_NODECLASS_VARIABLE) ? 0 : 1;

                // 设置名称
                if (result->displayName.length > 0) {
                    size_t name_len =
                        result->displayName.length < NEU_TAG_NAME_LEN
                    ...
                    size_t name_len =
                        result->browseName.name.length < NEU_TAG_NAME_LEN
                        ? result->browseName.name.length
                        : NEU_TAG_NAME_LEN - 1;
                    memcpy(scan_tag.name, result->browseName.name.data,
                           name_len);
                    scan_tag.name[name_len] = '\0';
                } else {
                ...
                if (result->nodeClass == UA_NODECLASS_VARIABLE) {
                    ...
                    // 使用改进的函数获取数据类型
                    if (opcua_client_read_variable_datatype(
                            plugin->client, &result->nodeId, &detected_type) ==
                        0) {
                        scan_tag.type = detected_type;

                        // 设置节点ID作为地址
                        char *node_id_str =
                            opcua_client_node_id_to_string(&result->nodeId);
                        if (node_id_str) {
                            strncpy(scan_tag.id, node_id_str,
                                    sizeof(scan_tag.id) - 1);
                        ...
                        if (opcua_client_read_value_attribute(
                                plugin->client, &result->nodeId, &value) ==
                            UA_STATUSCODE_GOOD) {
                            // 检查是否是数组类型
                            if (value.arrayLength > 0) {
                                ...
                                scan_tag.is_last_layer =
                                    false; // 设置为非最后一层
                            }
                            // 检查是否是ByteString类型
                            else if (value.type && value.type->typeName &&
                                     strcmp(value.type->typeName,
                                            "ByteString") == 0) {
                        ...
                        if (result->displayName.length > 0 &&
                            result->displayName.data != NULL) {
                            size_t copy_len = result->displayName.length <
                                    sizeof(name_buf) - 1
                                ? result->displayName.length
                                : sizeof(name_buf) - 1;
                            memcpy(name_buf, result->displayName.data,
                                   copy_len);
                            name_buf[copy_len] = '\0';
                        }
                        plog_debug(plugin,
                                   "Failed to read data type for node %s",
                                   name_buf);
                    ...
                    if (is_special_node) {
                        ...
                        if (temp_results != NULL) {
                            if (opcua_client_browse_node(plugin->client,
                                                         &result->nodeId,
                                                         temp_results) == 0) {
                        ...
                        !is_array_type) { // 如果不是数组类型且不是特殊节点，则按普通变量处理
                        // 普通变量节点是最后一层
                        scan_tag.is_last_layer = true;
                    }
