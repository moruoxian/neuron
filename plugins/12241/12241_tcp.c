...
static int update_tag_state(neu_plugin_t *plugin, const char *group_name, const char *tag_name, time_t update_time);
static int get_tag_state(neu_plugin_t *plugin, const char *group_name, const char *tag_name, tag_state_t **state);
static polling_group_t *parse_polling_tags(const char *json_str, int *group_count);
static void generate_polling_tasks_for_group(neu_plugin_t *plugin, neu_plugin_group_t *group);
static void execute_polling_tasks(neu_plugin_t *plugin);
static int gb_12241_build_his_request(neu_plugin_t *plugin, uint8_t *frame, size_t frame_size,
                                     uint16_t device_addr, uint8_t seq,
                                     uint16_t fn, uint16_t pn,
                                     time_t start_time, time_t end_time,
                                     size_t *request_length);
static int parse_and_store_historical_data(neu_plugin_t *plugin, const uint8_t *response, size_t response_len,
                                          const char *group_name, const char *tag_name,
                                          uint16_t fn, uint16_t pn, uint16_t data_index,
                                          time_t start_time, time_t end_time);
static int get_interval_seconds_from_density(uint8_t density);

// 新的基于tag_name+实时FN的映射函数声明
static uint16_t get_historical_fn_from_tag_and_fn(neu_plugin_t *plugin, const char *tag_name, uint16_t realtime_fn);
static const char* get_fn_mapping_description(neu_plugin_t *plugin, const char *tag_name, uint16_t realtime_fn);
static int init_fn_mapping_hash(neu_plugin_t *plugin);
static void cleanup_fn_mapping_hash(neu_plugin_t *plugin);


// 组级时间戳管理函数声明
static bool should_generate_polling_tasks(neu_plugin_t *plugin, const char *group_name);
...
static const rtu_tag_pattern_t rtu_patterns[] = {
    {"rtudi", "RTU数字量输入"},
    {"rtuai", "RTU模拟量输入"}, 
    {"rtuci", "RTU计数器输入"}
};
static const size_t rtu_patterns_count = sizeof(rtu_patterns) / sizeof(rtu_tag_pattern_t);
...
static const static_fn_mapping_t static_fn_mappings[] = {
    ...
    {"ForwardActiveEnergy_Total", 829, 801, "正向有功总电能(F829) -> 正向有功电能示值（总）曲线(F801)"},
    {"ForwardActiveEnergy_Sharp", 829, 809, "正向有功尖时电能(F829) -> 正向有功电能示值（尖）曲线(F809)"},
    {"ForwardActiveEnergy_Peak", 829, 810, "正向有功峰时电能(F829) -> 正向有功电能示值（峰）曲线(F810)"},
    {"ForwardActiveEnergy_Ground", 829, 811, "正向有功平时电能(F829) -> 正向有功电能示值（平）曲线(F811)"},
    {"ForwardActiveEnergy_Valley", 829, 812, "正向有功谷时电能(F829) -> 正向有功电能示值（谷）曲线(F812)"},
    
    // F145当月正向有功最大需量 -> 不支持
    {"MonthActiveMaxDemand", 145, 0, "当月正向有功最大需量(F145) -> 无对应历史功能码"},
    
    // F900有功最大需量 -> F900当前有功需量曲线
    {"ActiveDemand", 900, 900, "有功最大需量(F900) -> 当前有功需量曲线(F900)"},
    ...
    {"TotalFlow", 404, 401, "总累积流量(F404) -> 水表正向总累积流量曲线(F401)"},
    
    // F402水表状态字 -> 无对应历史功能码（状态字数据）
    {"PressureSensor_Status_Bit0", 402, 0, "压力传感器状态位0(F402) -> 无对应历史功能码"},
    {"PressureSensor_Status_Bit1", 402, 0, "压力传感器状态位1(F402) -> 无对应历史功能码"},
    {"FlowSensor_Status_Bit0", 402, 0, "流量传感器状态位0(F402) -> 无对应历史功能码"},
    {"FlowSensor_Status_Bit1", 402, 0, "流量传感器状态位1(F402) -> 无对应历史功能码"},
    ...
    {"MagneticDetection_Abnormal", 402, 0, "强磁监测(F402) -> 无对应历史功能码"},
    
    // ===== 气表类数据映射 =====
    // F503气表瞬时流量 -> 对应历史曲线
    {"StandardFlowRate", 503, 504, "标况瞬时流量(F503) -> 气表标况瞬时流量曲线(F504)"},
    {"WorkingFlowRate", 503, 505, "工况瞬时流量(F503) -> 气表工况瞬时流量曲线(F505)"},
    {"Pressure", 503, 502, "压力(F503) -> 气表压力曲线(F502)"},
    {"Temperature", 503, 503, "温度(F503) -> 气表温度曲线(F503)"},
    
    // F504气表累积流量 -> 对应历史曲线
    {"StandardTotalFlow", 504, 506, "标况累积流量(F504) -> 气表标况累积流量曲线(F506)"},
    {"WorkingTotalFlow", 504, 507, "工况累积流量(F504) -> 气表工况累积流量曲线(F507)"},
    
    // F502气表状态字 -> 无对应历史功能码（状态字数据）
    {"TempSensor_Status_Bit0", 502, 0, "温度传感器状态位0(F502) -> 无对应历史功能码"},
    {"TempSensor_Status_Bit1", 502, 0, "温度传感器状态位1(F502) -> 无对应历史功能码"},
    ...
    {"AccumFlow", 603, 612, "累积流量(F603) -> 热量表正向总累积流量曲线(F612)"},
    {"AccumHeat", 603, 611, "累积热量(F603) -> 热量表正向总累积热量曲线(F611)"},
...
static int gb_12241_build_multi_request(neu_plugin_t *plugin, uint8_t *frame, size_t frame_size, 
                               ...
                               size_t *request_length);
//对时
static int gb_12241_build_time_sync_request(neu_plugin_t *plugin, uint8_t *frame, size_t frame_size,
                                           uint16_t device_addr, uint8_t seq,
                                           size_t *request_length);
//执行对时
...
static bool is_rtu_tag(const char *tag_name) {
    ...
    for (size_t i = 0; i < rtu_patterns_count; i++) {
        if (strncmp(tag_name, rtu_patterns[i].pattern, strlen(rtu_patterns[i].pattern)) == 0) {
            return true;
        }
    }
 ...
 */
static int get_rtu_tag_order_in_group(neu_plugin_t *plugin, const char *group_name, const char *tag_name) {
    ...
    plog_warn(plugin, "RTU标签 '%s' 在组'%s'中未找到映射", tag_name, group_name);
    return -1;
}
/**
 ...
 */
static uint16_t get_rtu_historical_fn(neu_plugin_t *plugin, const char *group_name, 
                                     const char *tag_name, uint16_t realtime_fn) {
    ...
    if (rtu_order >= 0 && rtu_order < 12) {
        ...
        plog_warn(plugin, "RTU标签 '%s' 在组'%s'中顺序无效: %d", tag_name, group_name, rtu_order);
        return 0;
    }
}
...
static int build_rtu_mapping_table(neu_plugin_t *plugin) {
    ...
    for (int i = 0; i < plugin->polling_group_count && build_success; i++) {
        ...
        for (int j = 0; j < plugin->polling_groups[i].tag_count && build_success; j++) {
            const char *tag_name = plugin->polling_groups[i].tags[j];
            
            if (is_rtu_tag(tag_name)) {
                if (rtu_order < 12) {
                    // 创建映射条目
                    rtu_mapping_cache_t *mapping = calloc(1, sizeof(rtu_mapping_cache_t));
                    if (!mapping) {
                    ...
                    strncpy(mapping->group_name, group_name, NEU_GROUP_NAME_LEN - 1);
                    ...
                    plog_debug(plugin, "构建RTU映射: %s.%s -> 顺序%d, FN=%u", 
                              group_name, tag_name, rtu_order, mapping->historical_fn);
                    
                    rtu_order++;
                } else {
                    plog_warn(plugin, "RTU标签 '%s' 在组'%s'中顺序超出限制: %d (最大11)", 
...
static int plugin_config(neu_plugin_t *plugin, const char *config)
{
    ...
    neu_json_elem_t polling_enabled_elem = { .name = "polling_enabled", .t = NEU_JSON_INT };
    neu_json_elem_t max_poll_age_elem = { .name = "max_poll_age", .t = NEU_JSON_INT };
    neu_json_elem_t current_before_time_elem = { .name = "current_before_time", .t = NEU_JSON_INT };
    neu_json_elem_t polling_tags_elem = { .name = "polling_tags", .t = NEU_JSON_STR };
    // ++++++++++++++++++++++++++++++++
    ...
    // ++++ 设置召测配置 ++++
    // 从配置文件中读取召测相关参数
    ret = neu_parse_param((char *) config, &err_param, 4, 
                         &polling_enabled_elem, &max_poll_age_elem, 
                         &current_before_time_elem, &polling_tags_elem);
    ...
    if (ret != 0) {
        ...
        plog_notice(plugin, "召测配置 - 启用: %s, 最大时间范围: %d秒, 当前时间偏移: %d秒",
                   plugin->polling_enabled ? "是" : "否",
                   plugin->max_poll_age_sec,
                   plugin->current_before_time_sec);
        
        if (plugin->polling_tags_str != NULL) {
            plog_notice(plugin, "召测配置 - 标签列表: %s", plugin->polling_tags_str);
        } else {
                                  ...
                                  uint16_t *pn, uint16_t *data_index)
{
    ...
    if (ret == 4) {
        ...
        *pn = (uint16_t)param;
        ...
        plog_debug(plugin, "解析地址成功(新格式4): %s -> 设备号=%u, 功能码=%u, 参数号=%u, 数据索引=%u", 
                ...
                for (int i = 0; i < map_size; i++) {
                    ...
                    if (tag_map[i].fn == recv_fn && tag_map[i].pn == recv_pn) {
                        ...
                        if (gb_12241_extract_data_value(plugin, current_pos, data_unit_size, 
                            ...
                            update_tag_state(plugin, group->group_name, tag->name, now);
                            tags_updated++;
                        }
                    }
                }
...
static int driver_stop(neu_plugin_t *plugin)
{
    ...
    if (plugin->tag_state_db) {
        ...
        char *errmsg = NULL;
        int rc = sqlite3_exec(plugin->tag_state_db, "BEGIN TRANSACTION;", NULL, NULL, &errmsg);
        if (rc != SQLITE_OK) {
            plog_error(plugin, "开启事务失败: %s", errmsg ? errmsg : sqlite3_errmsg(plugin->tag_state_db));
        ...
        HASH_ITER(hh, plugin->tag_states, state, tmp) {
            if (state->dirty) {
                const char *sql = "INSERT OR REPLACE INTO tag_state (group_name, tag_name, last_update_time, last_polled_historical_time, dirty) VALUES (?, ?, ?, ?, ?);";
                sqlite3_stmt *stmt;
                rc = sqlite3_prepare_v2(plugin->tag_state_db, sql, -1, &stmt, NULL);
                if (rc == SQLITE_OK) {
                    sqlite3_bind_text(stmt, 1, state->group_name, -1, SQLITE_STATIC);
                    sqlite3_bind_text(stmt, 2, state->tag_name, -1, SQLITE_STATIC);
                    sqlite3_bind_int64(stmt, 3, state->last_update_time);
                    sqlite3_bind_int64(stmt, 4, state->last_polled_historical_time);
        ...
        if (rc != SQLITE_OK) {
            plog_error(plugin, "提交事务失败: %s", errmsg ? errmsg : sqlite3_errmsg(plugin->tag_state_db));
            if (errmsg) sqlite3_free(errmsg);
            // 回滚事务
    ...
    if (plugin->tag_state_db && plugin->polling_tasks) {
        ...
        int rc = sqlite3_exec(plugin->tag_state_db, "BEGIN TRANSACTION;", NULL, NULL, &errmsg);
        if (rc != SQLITE_OK) {
            plog_error(plugin, "开启polling_task事务失败: %s", errmsg ? errmsg : sqlite3_errmsg(plugin->tag_state_db));
            if (errmsg) sqlite3_free(errmsg);
            pthread_mutex_unlock(&plugin->polling_task_mutex);
            // 继续执行，不中断停止流程
        } else {
            // 先清空现有任务
            rc = sqlite3_exec(plugin->tag_state_db, "DELETE FROM polling_task;", NULL, NULL, &errmsg);
            if (rc != SQLITE_OK) {
                plog_error(plugin, "清空polling_task表失败: %s", errmsg ? errmsg : sqlite3_errmsg(plugin->tag_state_db));
                if (errmsg) sqlite3_free(errmsg);
                sqlite3_exec(plugin->tag_state_db, "ROLLBACK;", NULL, NULL, NULL);
                pthread_mutex_unlock(&plugin->polling_task_mutex);
                // 继续执行，不中断停止流程
                ...
                for (unsigned int i = 0; i < total_tasks; i++) {
                    polling_task_t *task = (polling_task_t *)utarray_eltptr(plugin->polling_tasks, i);
                    if (task) {
                        const char *sql = "INSERT INTO polling_task (group_name, tag_name, tag_address, start_time, end_time, retry_count) VALUES (?, ?, ?, ?, ?, ?);";
                        sqlite3_stmt *stmt;
                        rc = sqlite3_prepare_v2(plugin->tag_state_db, sql, -1, &stmt, NULL);
                        if (rc == SQLITE_OK) {
                            sqlite3_bind_text(stmt, 1, task->group_name, -1, SQLITE_STATIC);
                            sqlite3_bind_text(stmt, 2, task->tag_name, -1, SQLITE_STATIC);
                            sqlite3_bind_text(stmt, 3, task->tag_address, -1, SQLITE_STATIC);
                            ...
                            if (rc == SQLITE_DONE) {
                                ...
                                plog_error(plugin, "插入polling_task失败 [%s.%s]: %s", 
                                         task->group_name, task->tag_name, 
                                         sqlite3_errmsg(plugin->tag_state_db));
                            }
                            sqlite3_finalize(stmt);
                        } else {
                            plog_error(plugin, "准备polling_task SQL语句失败: %s", 
                                     sqlite3_errmsg(plugin->tag_state_db));
                        }
                    }
                }
                ...
                rc = sqlite3_exec(plugin->tag_state_db, "COMMIT;", NULL, NULL, &errmsg);
                if (rc != SQLITE_OK) {
                    plog_error(plugin, "提交polling_task事务失败: %s", errmsg ? errmsg : sqlite3_errmsg(plugin->tag_state_db));
                    if (errmsg) sqlite3_free(errmsg);
                    sqlite3_exec(plugin->tag_state_db, "ROLLBACK;", NULL, NULL, NULL);
                } else {
                    plog_notice(plugin, "批量持久化polling_task任务 %d 条 (总共 %d 条)", task_count, total_tasks);
                }
                if (errmsg) sqlite3_free(errmsg);
            }
        }
...
static int driver_group_timer(neu_plugin_t *plugin, neu_plugin_group_t *group)
{
    ...
    if (plugin->polling_enabled && should_generate_polling_tasks(plugin, safe_group_name)) {
...
static int gb_12241_execute_time_sync(neu_plugin_t *plugin)
{
    ...
    // 检查是否需要对时 - 每1小时执行一次
    if (plugin->last_time_sync != 0 && (current_time - plugin->last_time_sync) < 60) {
        return 0; // 无需对时
    }
    ...
    int ret = gb_12241_build_time_sync_request(plugin, time_sync_frame, sizeof(time_sync_frame),
                                               device_addr, plugin->seq++, &frame_len);
    if (ret != 0) {
    ...
    size_t response_len = sizeof(response);
    ret = gb_12241_send_and_receive(plugin, time_sync_frame, frame_len, response, &response_len);
                                           ...
                                           size_t *request_length)
{
    ...
    *buf = AFN_CMD;
    ...
    uint16_t calc_len = ((uint16_t)link_len.LUSERH << 6) | link_len.LUSERL;
    if (calc_len != user_data_len) {
        plog_error(plugin, "对时请求长度计算错误: 预期=%u, 实际=%u", user_data_len, calc_len);
        return -1;
    }
    ...
    *request_length = (size_t)(buf - frame);
    ...
    plog_notice(plugin, "对时请求帧构建完成: 总长度=%zu, 设备地址=%u, 时间=%04d-%02d-%02d %02d:%02d:%02d", 
              *request_length, device_addr,
                                     ...
                                     uint16_t fn, uint8_t data_index, neu_value_u *value)
{
    ...
    switch (fn) {
            ...
            return 1; // 返回1表示数据存在但无效
                ...
                case 145: { // F145：当月正向有功最大需量及发生时间
                    ...
                    if (data_index == 0) {
                        ...
                        Data_Type_23 *demand_total =
                            (Data_Type_23 *) (data + 6);

                        // 检查需量值数据有效性
                        if (data_type_23_getflag(demand_total)) {
                            double demand_value =
                                data_type_23_getvalue(demand_total);
                            value->d64 = demand_value;
                            plog_debug(plugin, "F145 总最大需量: %.4f kW",
                                       demand_value);
                            return 0;
                        }
                        ...
                        plog_error(
                            plugin,
                            "F145 数据索引超出范围: %d (仅支持data_index=0)",
                            data_index);
                        return -1;
                    }
                    break;
        ...
        {
            if (data_unit_size < 6) { // 至少需要5字节时标 + 1字节费率
                plog_error(plugin, "FN %u 数据长度不足: 期望至少6字节, 实际%d字节", fn, data_unit_size);
                return -1;
            }
            ...
            plog_debug(plugin, "FN %u: 费率数量=%u, data_index=%u", fn, tarrif, data_index);
            ...
            if (data_type_39_getflag(target_data39)) {
                ...
                plog_debug(plugin, "FN %u data_index=%u: 电能值=%.4f", fn, data_index, energy_value);
                return 0;
            }
...
static int init_tag_state_db(neu_plugin_t *plugin)
{
    ...
    // 构建数据库文件路径
    snprintf(db_path, sizeof(db_path), "persistence/12241_%s.db", plugin->common.name);

    // 打开数据库连接
    rc = sqlite3_open(db_path, &plugin->tag_state_db);
    if (rc != SQLITE_OK) {
        plog_error(plugin, "Cannot open database: %s", sqlite3_errmsg(plugin->tag_state_db));
    ...
    const char *sql_create_polling_task =
        ...
        ");"
        "CREATE INDEX IF NOT EXISTS idx_polling_task_group_tag ON polling_task(group_name, tag_name);";
    rc = sqlite3_exec(plugin->tag_state_db, sql_create_polling_task, 0, 0, &err_msg);
    if (rc != SQLITE_OK) {
    ...
    plugin->polling_task_icd = (UT_icd){sizeof(polling_task_t), NULL, NULL, NULL};
    ...
    // 从数据库加载所有点位状态到内存
    sql = "SELECT group_name, tag_name, last_update_time, last_polled_historical_time, dirty FROM tag_state;";
    sqlite3_stmt *stmt;
    rc = sqlite3_prepare_v2(plugin->tag_state_db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        plog_error(plugin, "Failed to prepare statement: %s", sqlite3_errmsg(plugin->tag_state_db));
        return -1;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        strncpy(state->group_name, (const char *)sqlite3_column_text(stmt, 0), NEU_GROUP_NAME_LEN - 1);
        strncpy(state->tag_name, (const char *)sqlite3_column_text(stmt, 1), NEU_TAG_NAME_LEN - 1);
        ...
        HASH_ADD(hh, plugin->tag_states, group_name, NEU_GROUP_NAME_LEN + NEU_TAG_NAME_LEN, state);
    }

    sqlite3_finalize(stmt);

    // 加载历史召测任务到UT_array
    const char *sql_select = "SELECT group_name, tag_name, tag_address, start_time, end_time, retry_count FROM polling_task;";
    rc = sqlite3_prepare_v2(plugin->tag_state_db, sql_select, -1, &stmt, NULL);
    if (rc == SQLITE_OK) {
        pthread_mutex_lock(&plugin->polling_task_mutex);
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            polling_task_t task = {0};
            strncpy(task.group_name, (const char *)sqlite3_column_text(stmt, 0), NEU_GROUP_NAME_LEN-1);
            strncpy(task.tag_name, (const char *)sqlite3_column_text(stmt, 1), NEU_TAG_NAME_LEN-1);
            strncpy(task.tag_address, (const char *)sqlite3_column_text(stmt, 2), NEU_TAG_ADDRESS_LEN-1);  //
        ...
        pthread_mutex_unlock(&plugin->polling_task_mutex);
        sqlite3_finalize(stmt);
        plog_notice(plugin, "历史召测任务加载完成，共%u条", (unsigned)utarray_len(plugin->polling_tasks));
    } else {
        plog_error(plugin, "prepare polling_task select failed: %s", sqlite3_errmsg(plugin->tag_state_db));
    }
...
static int update_tag_state(neu_plugin_t *plugin, const char *group_name, const char *tag_name, time_t update_time)
{
    ...
    HASH_FIND(hh, plugin->tag_states, &key, NEU_GROUP_NAME_LEN + NEU_TAG_NAME_LEN, state);
...
static polling_group_t *parse_polling_tags(const char *json_str, int *group_count) {
    if (!json_str || strlen(json_str) == 0) return NULL;
    ...
    for (int i = 0; i < n; i++) {
        ...
        neu_json_elem_t fields[2] = {
            { .name = "group", .t = NEU_JSON_STR },
            { .name = "tags",  .t = NEU_JSON_ARRAY_STR }
        };
        ...
        if (!fields[0].v.val_str || fields[1].t != NEU_JSON_ARRAY_STR || fields[1].v.val_array_str.length <= 0) {
            neu_json_elem_free(&fields[0]);
            neu_json_elem_free(&fields[1]);
            continue;
        }
...
static void execute_polling_tasks(neu_plugin_t *plugin) {
    ...
    unsigned int tasks_to_process = (task_count < max_tasks_per_batch) ? task_count : max_tasks_per_batch;
    ...
    for (unsigned int task_idx = 0; task_idx < tasks_to_process; task_idx++) {
        ...
        strftime(timebuf_start, sizeof(timebuf_start), "%Y-%m-%d %H:%M:%S", &tm_start);
        strftime(timebuf_end, sizeof(timebuf_end), "%Y-%m-%d %H:%M:%S", &tm_end);
        
        plog_notice(plugin, "执行历史召测 [%d/%d]: %s.%s [%s~%s] (重试%d次)", 
                   task_idx + 1, tasks_to_process,
                   current_task->group_name, current_task->tag_name, 
                   timebuf_start, timebuf_end, current_task->retry_count);
        ...
        if (gb_12241_parse_tag_address(plugin, tag_address, 
                                     &device_addr, &fn, &pn, &data_index) != 0) {
            plog_error(plugin, "历史召测失败: 无法解析标签地址 %s", tag_address);
            continue;
        }
        ...
        if (historical_fn == 0) {
            plog_warn(plugin, "历史召测跳过: 标签名称'%s'无对应历史功能码，标签=%s.%s", 
                     current_task->tag_name, current_task->group_name, current_task->tag_name);
            continue;
        }
        ...
        const char *mapping_desc = get_fn_mapping_description(plugin,current_task->tag_name, realtime_fn);
        plog_notice(plugin, "历史召测FN映射: %s.%s 实时FN=%u -> 历史FN=%u (%s)", 
                   current_task->group_name, current_task->tag_name, 
                   realtime_fn, historical_fn, mapping_desc ? mapping_desc : "未知映射");
        
        // 构建历史召测请求
        uint8_t request[1024];
        size_t request_len = 0;
        
        int ret = gb_12241_build_his_request(plugin, request, sizeof(request),
                                            device_addr, plugin->seq++,
                                            fn, pn,
                                            current_task->start_time, current_task->end_time,
                                            &request_len);
        
        if (ret != 0) {
            plog_error(plugin, "历史召测失败: 构建请求帧失败，设备=%d, FN=%d, PN=%d", 
                      device_addr, fn, pn);
            continue;
        }
        ...
        ret = gb_12241_send_and_receive(plugin, request, request_len, response, &response_len);
        if (ret != 0) {
            plog_error(plugin, "历史召测失败: 发送接收失败，设备=%d, FN=%d, PN=%d", 
                      device_addr, fn, pn);
            continue;
        }
        ...
        // 解析历史数据响应并存储
        ret = parse_and_store_historical_data(plugin, response, response_len,
                                             current_task->group_name, current_task->tag_name,
                                             fn, pn, data_index,
                                             current_task->start_time, current_task->end_time);
    ...
    uint16_t data_index, time_t start_time, time_t end_time)
{
...
else if (recv_fn == 502 || recv_fn == 503) { // 气表压力和温度曲线 (FN 502-503)
    ...
    for (int j = 0; j < td.Nums; j++) {
        ...
        if (data_type_37_getflag(&data_array[j])) { // ✅ 对应您C++代码的getflag()
            float value = data_type_37_getvalue(&data_array[j]); // ✅ 对应您C++代码的getvalue()

            char       time_str[32];
            struct tm *tm_info = localtime(&current_timestamp);
            strftime(time_str, sizeof(time_str), "%Y-%m-%d %H:%M:%S", tm_info);

            const char *curve_type = (recv_fn == 502) ? "气表压力" : "气表温度"; // ✅ 正确区分
            
            plog_notice(plugin,
                        "%s曲线[%d/%d]: %s.%s 时间=%s, 值=%.3f, FN=%u, PN=%u",
                        curve_type, j + 1, td.Nums, group_name, tag_name,
                        time_str, value, fn, pn);

            // 更新点位状态
            update_tag_state(plugin, group_name, tag_name, current_timestamp);

            valid_count++;
        } else {
            const char *curve_type = (recv_fn == 502) ? "气表压力" : "气表温度";
            plog_debug(plugin, "%s曲线[%d/%d]: 数据无效", curve_type, j + 1, td.Nums);
        }
    }

    current_pos += expected_data_len; // ✅ 对应您C++代码的 i += sizeof(Data_Type_37) * td.Nums
    remaining_len -= expected_data_len;

    const char *curve_type = (recv_fn == 502) ? "气表压力" : "气表温度";
    plog_notice(plugin, "%s曲线解析完成: %s.%s, 总数=%d, 有效=%d",
                curve_type, group_name, tag_name, td.Nums, valid_count);
}
else if (recv_fn == 504 || recv_fn == 505) { // 气表瞬时流量曲线 (FN 504-505)
    ...
    for (int j = 0; j < td.Nums; j++) {
        ...
        if (data_type_29_getflag(&data_array[j])) { // ✅ 对应您C++代码的getflag()
            float value = data_type_29_getvalue(&data_array[j]); // ✅ 对应您C++代码的getvalue()
            ...
            // ✅ 正确对应您C++代码的标况/工况区分
            const char *flow_type = (recv_fn == 504) ? "气表标况瞬时流量" : "气表工况瞬时流量";
            
            plog_notice(plugin,
                        "%s曲线[%d/%d]: %s.%s 时间=%s, 值=%.3f, FN=%u, PN=%u",
                        flow_type, j + 1, td.Nums, group_name, tag_name,
                        time_str, value, fn, pn);
            ...
            const char *flow_type = (recv_fn == 504) ? "气表标况瞬时流量" : "气表工况瞬时流量";
            plog_debug(plugin, "%s曲线[%d/%d]: 数据无效", flow_type, j + 1, td.Nums);
        }
    }

    current_pos += expected_data_len; // ✅ 对应您C++代码的 i += sizeof(Data_Type_29) * td.Nums
    remaining_len -= expected_data_len;

    const char *flow_type = (recv_fn == 504) ? "气表标况瞬时流量" : "气表工况瞬时流量";
    plog_notice(plugin, "%s曲线解析完成: %s.%s, 总数=%d, 有效=%d",
                flow_type, group_name, tag_name, td.Nums, valid_count);
} else if (recv_fn == 506 || recv_fn == 507) { // 气表累积流量曲线 (FN 506-507)
    ...
    for (int j = 0; j < td.Nums; j++) {
        ...
        if (data_type_38_getflag(&data_array[j])) { // ✅ 对应您C++代码的getflag()
            float value = data_type_38_getvalue(&data_array[j]); // ✅ 对应您C++代码的getvalue()
            ...
            // ✅ 正确区分标况累积流量和工况累积流量
            const char *flow_type = (recv_fn == 506) ? "气表标况累积流量" : "气表工况累积流量";
            
            plog_notice(plugin,
                        "%s曲线[%d/%d]: %s.%s 时间=%s, 值=%.6f, FN=%u, PN=%u",
                        flow_type, j + 1, td.Nums, group_name, tag_name,
                        time_str, value, fn, pn);
    ...
    current_pos += expected_data_len; // ✅ 对应您C++代码的 i += sizeof(Data_Type_38) * td.Nums
    remaining_len -= expected_data_len;
...
else if (recv_fn == 607 || recv_fn == 608) { // 热表进水/回水温度曲线 (FN 607-608)
    ...
    for (int j = 0; j < td.Nums; j++) {
        ...
        if (data_type_36_getflag(&data_array[j])) { // ✅ 对应您C++代码的getflag()
            float value = data_type_36_getvalue(&data_array[j]); // ✅ 对应您C++代码的getvalue()
            ...
            // ✅ 正确区分进水温度和回水温度
            const char *temp_type = (recv_fn == 607) ? "热表进水温度" : "热表回水温度";
            
            plog_notice(plugin,
                        "%s曲线[%d/%d]: %s.%s 时间=%s, 值=%.2f℃, FN=%u, PN=%u",
                        temp_type, j + 1, td.Nums, group_name, tag_name,
                        time_str, value, fn, pn);
            ...
            const char *temp_type = (recv_fn == 607) ? "热表进水温度" : "热表回水温度";
            plog_debug(plugin, "%s曲线[%d/%d]: 数据无效", temp_type, j + 1, td.Nums);
        }
    }

    current_pos += expected_data_len; // ✅ 对应您C++代码的 i += sizeof(Data_Type_36) * td.Nums
    remaining_len -= expected_data_len;

    const char *temp_type = (recv_fn == 607) ? "热表进水温度" : "热表回水温度";
    plog_notice(plugin, "%s曲线解析完成: %s.%s, 总数=%d, 有效=%d",
                temp_type, group_name, tag_name, td.Nums, valid_count);
}
    ...
    for (int j = 0; j < td.Nums; j++) {
        ...
        if (data_type_29_getflag(&data_array[j])) { // ✅ 对应您C++代码的getflag()
            ...
            plog_notice(plugin,
                        "热表瞬时流量曲线[%d/%d]: %s.%s 时间=%s, 值=%.3f, FN=%u, PN=%u",
                        j + 1, td.Nums, group_name, tag_name,
                        time_str, value, fn, pn);
            ...
            plog_debug(plugin, "热表瞬时流量曲线[%d/%d]: 数据无效", j + 1, td.Nums);
        }
    }
    ...
    plog_notice(plugin, "热表瞬时流量曲线解析完成: %s.%s, 总数=%d, 有效=%d",
                group_name, tag_name, td.Nums, valid_count);
} else if (recv_fn == 611) { // 热量表正向总累积热量曲线 (FN 611)
    // 热量表正向总累积热量曲线处理 - 使用Data_Type_38
    size_t expected_data_len = td.Nums * sizeof(Data_Type_38);
    if (remaining_len < expected_data_len) {
        plog_error(plugin, "数据长度不足 (热量表累积热量曲线 FN 611): 期望%zu字节, 实际%zu字节",
                   expected_data_len, remaining_len);
        break;
    }
    ...
    for (int j = 0; j < td.Nums; j++) {
        ...
        if (data_type_38_getflag(&data_array[j])) {
            ...
            plog_notice(plugin,
                        "热量表正向总累积热量曲线[%d/%d]: %s.%s 时间=%s, 值=%.6f, FN=%u, PN=%u",
                        j + 1, td.Nums, group_name, tag_name, time_str, value, fn, pn);

            // 更新点位状态
            update_tag_state(plugin, group_name, tag_name, current_timestamp);
            valid_count++;
        } else {
            plog_debug(plugin, "热量表正向总累积热量曲线[%d/%d]: 数据无效", j + 1, td.Nums);
        }
    }
    ...
    plog_notice(plugin, "热量表正向总累积热量曲线解析完成: %s.%s, 总数=%d, 有效=%d",
                group_name, tag_name, td.Nums, valid_count);
    ...
    size_t expected_data_len = td.Nums * sizeof(Data_Type_38);
    if (remaining_len < expected_data_len) {
        plog_error(plugin, "数据长度不足 (热量表累积流量曲线 FN 612): 期望%zu字节, 实际%zu字节",
                   expected_data_len, remaining_len);
        break;
    }
    ...
    for (int j = 0; j < td.Nums; j++) {
        ...
        if (data_type_38_getflag(&data_array[j])) {
            ...
            plog_notice(plugin,
                        "热量表正向总累积流量曲线[%d/%d]: %s.%s 时间=%s, 值=%.6f, FN=%u, PN=%u",
                        j + 1, td.Nums, group_name, tag_name, time_str, value, fn, pn);
            ...
            plog_debug(plugin, "热量表正向总累积流量曲线[%d/%d]: 数据无效", j + 1, td.Nums);
        }
    }
    ...
    plog_notice(plugin, "热量表正向总累积流量曲线解析完成: %s.%s, 总数=%d, 有效=%d",
                group_name, tag_name, td.Nums, valid_count);
} else if (recv_fn >= 710 && recv_fn <= 721) { // RTU曲线数据 (FN 710-721)
    // RTU曲线数据处理 - 统一使用Data_Type_40
    
    uint16_t data_type_index = recv_fn - 710; // 0-11对应Data0-Data11
    
    plog_notice(plugin, "RTU曲线数据解析开始: %s.%s, FN=%u (Data%u曲线), PN=%u, 数量=%d",
                group_name, tag_name, recv_fn, data_type_index, recv_pn, td.Nums);
    
    // 所有RTU曲线数据都使用Data_Type_40
    size_t expected_data_len = td.Nums * sizeof(Data_Type_40);
    if (remaining_len < expected_data_len) {
        plog_error(plugin, "数据长度不足 (RTU Data%u曲线 FN %u): 期望%zu字节, 实际%zu字节",
                   data_type_index, recv_fn, expected_data_len, remaining_len);
        break;
    }
    ...
    for (int j = 0; j < td.Nums; j++) {
        ...
        if (data_type_40_getflag(&data_array[j])) {
            double value = (double)data_type_40_getvalue(&data_array[j]);  // ← 修正：使用double
            ...
            plog_notice(plugin,
                        "RTU Data%u曲线[%d/%d]: %s.%s 时间=%s, 值=%.6f, FN=%u, PN=%u",  // ← %.6f适用于double
                        data_type_index, j + 1, td.Nums, group_name, tag_name, 
                        time_str, value, recv_fn, recv_pn);
    ...
    current_pos += expected_data_len; // 对应您C++代码的 i += sizeof(Data_Type_40) * td.Nums
    remaining_len -= expected_data_len;
    
    plog_notice(plugin, "RTU Data%u曲线解析完成: %s.%s, 总数=%d, 有效=%d",
                data_type_index, group_name, tag_name, td.Nums, valid_count);
}
 ...
 */
static void generate_polling_tasks_for_group(neu_plugin_t *plugin, neu_plugin_group_t *group) {
    if (!plugin->polling_enabled || !group) return;
    ...
    utarray_foreach(group->tags, neu_datatag_t *, tag) {
                ...
                if (strcmp(plugin->polling_groups[i].group, group_name) == 0) {
                    for (int j = 0; j < plugin->polling_groups[i].tag_count; j++) {
                        if (plugin->polling_groups[i].tags[j] && 
                            strcmp(plugin->polling_groups[i].tags[j], tag->name) == 0) {
                            is_polling_tag = true;
                            break;
                        }
                    }
        ...
        tag_state_t *state = NULL;
        if (get_tag_state(plugin, group_name, tag->name, &state) != 0) continue;

        time_t last_poll = state->last_polled_historical_time;
        time_t last_update = state->last_update_time;

        // 如果已经采集到最新数据，则跳过
        if (before_time <= last_poll) continue;
        ...
        for (time_t t = start_time; t < before_time; t += POLL_INTERVAL_SEC) {
            ...
            if (task_end > before_time) task_end = before_time;

            // 只召测未更新到的数据
            if (task_end <= last_update) continue;
            ...
            strncpy(new_task.tag_address, tag->address, NEU_TAG_ADDRESS_LEN-1);  // 直接获取标签地址！
            ...
            if (utarray_len(plugin->polling_tasks) >= max_queue_size) {
                plog_warn(plugin, "召测任务队列已满(%u个)，丢弃最早的任务", max_queue_size);
    ...
    if (total_tasks_generated > 0) {
        plog_notice(plugin, "组 '%s' 生成召测任务: %d个, 当前队列总数: %u个", 
                   group_name, total_tasks_generated, (unsigned)utarray_len(plugin->polling_tasks));
    }
}
...
static bool should_generate_polling_tasks(neu_plugin_t *plugin, const char *group_name) {
    ...
    if (!group_time) {
        ...
        if (now - group_time->last_polling_time >= polling_interval) {
            group_time->last_polling_time = now;
            should_generate = true;
            plog_debug(plugin, "组 '%s' 满足时间间隔，生成召测任务", group_name);
        } else {
            plog_debug(plugin, "组 '%s' 时间间隔不足，跳过召测任务生成 (距离上次 %ld 秒)", 
...
static const size_t static_fn_mappings_size = sizeof(static_fn_mappings) / sizeof(static_fn_mapping_t);
...
static int init_fn_mapping_hash(neu_plugin_t *plugin) {
    ...
    for (size_t i = 0; i < static_fn_mappings_size; i++) {
        ...
        strncpy(entry->key.tag_name, static_fn_mappings[i].tag_name, NEU_TAG_NAME_LEN - 1);
        ...
        HASH_ADD(hh, plugin->fn_mapping_hash, key, sizeof(fn_mapping_key_t), entry);
    }
    
    plugin->fn_mapping_initialized = true;
    pthread_mutex_unlock(&plugin->fn_mapping_mutex);
    
    plog_debug(plugin, "实例级FN映射哈希表初始化完成，条目数量: %zu", static_fn_mappings_size);
    return 0;
}
...
static uint16_t get_historical_fn_from_tag_and_fn(neu_plugin_t *plugin, const char *tag_name, uint16_t realtime_fn) {
    ...
    if (is_rtu_tag(tag_name)) {
        ...
        plog_warn(plugin, "RTU标签 '%s' 需要使用 get_rtu_historical_fn 函数，需要组名信息", tag_name);
    ...
    HASH_FIND(hh, plugin->fn_mapping_hash, &key, sizeof(fn_mapping_key_t), entry);
    
    uint16_t historical_fn = 0;
    if (entry) {
        historical_fn = entry->historical_fn;
        plog_debug(plugin, "标签映射: %s(F%u) -> F%u (%s)", 
                  tag_name, realtime_fn, historical_fn, entry->description);
    } else {
        plog_warn(plugin, "未找到标签 '%s' + 实时FN=%u 的历史映射", tag_name, realtime_fn);
    }
...
static const char* get_fn_mapping_description(neu_plugin_t *plugin, const char *tag_name, uint16_t realtime_fn) {
    ...
    pthread_mutex_lock(&plugin->fn_mapping_mutex);
    HASH_FIND(hh, plugin->fn_mapping_hash, &search_key, sizeof(fn_mapping_key_t), entry);
    pthread_mutex_unlock(&plugin->fn_mapping_mutex);
...
static int get_interval_seconds_from_density(uint8_t density)
{
    switch (density)
    {
    case 1:		return 15 * 60;	break;
    case 2:		return 30 * 60;	break;
    case 3:		return 60 * 60;	break;
    case 254:	return 5 * 60;	break;
    case 255:	return 1 * 60;	break;
